...
const cancelQueueFile = async (file, showMessage = true) => {
  ...
  if (file.uploadId) {
    await fileApi.discardFailedUploads([file.uploadId]).catch(() => {})
    removeUploadDraft(file.uploadId)
  } else if (file.status === 'failed') {
    await fileApi.clearFailedUploads().catch(() => {})
  }
  if (showMessage) {
...
/**
 * 页面离开前清理失败项并标记可恢复上传中断。
 */
const cleanupQueueBeforeLeave = () => {
  ...
  fileApi.interruptUploads(interruptedUploadIds).catch(() => {})
}
